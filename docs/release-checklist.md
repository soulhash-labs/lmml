...
`-DGGML_HIP=ON -DGPU_TARGETS=...` CMake flags, settings persistence,
`rocm-smi`/`amd-smi` VRAM polling, and tests. Do not claim complete AMD
production telemetry until ROCm VRAM polling is validated on target AMD GPU
hosts.
