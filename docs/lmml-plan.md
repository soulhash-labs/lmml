...
- Live llama.cpp validation: use installed lmml to run a clean CUDA build, start
...
- `LMML_FIX_DEPS=1` may install only narrow apt prerequisites
  (compiler/cmake/git/curl/sccache). It must not auto-install Rust, NVIDIA
  drivers, CUDA, or broad GPU stacks.
